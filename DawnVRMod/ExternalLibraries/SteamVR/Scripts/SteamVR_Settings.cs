...
#if UNITY_EDITOR
                MelonLoader.MelonLogger.Msg("[SteamVR Setup] Generated you an editor app key of: " + _instance.editorAppKey + ". This lets the editor tell SteamVR what project this is. Has no effect on builds. This can be changed in Assets/SteamVR/Resources/SteamVR_Settings");
        ...
        private static GameObject FindDefaultPreviewHand(string assetName)
        {
            ...
            if (defaultPaths != null && defaultPaths.Length > 0)
            {
                ...
                if (defaultAsset == null)
                    MelonLoader.MelonLogger.Error("[SteamVR] Could not load default hand preview prefab: " + assetName + ". Found path: " + defaultPath);

                return defaultAsset;
            }
            //else //todo: this will generally fail on the first try but will try again before its an issue.
                //MelonLoader.MelonLogger.Error("[SteamVR] Could not load default hand preview prefab: " + assetName);
#endif
