...
namespace Valve.VR
{
    ...
    {
#if REMASTER
        public SteamVR_Skybox(System.IntPtr ptr) : base(ptr) { }
#endif
            ...
            Texture bottom = null)
        {
            ...
            if (compositor != null)
            {
                ...
                {
                    MelonLoader.MelonLogger.Error("[SteamVR] Failed to set skybox override with error: " + error);
                    if (error == EVRCompositorError.TextureIsOnWrongDevice)
                        MelonLoader.MelonLogger.Msg("[SteamVR] Set your graphics driver to use the same video card as the headset is plugged into for Unity.");
                    else if (error == EVRCompositorError.TextureUsesUnsupportedFormat)
                        MelonLoader.MelonLogger.Msg("[SteamVR] Ensure skybox textures are not compressed and have no mipmaps.");
                }
            }
        }
