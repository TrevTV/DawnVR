...
namespace Valve.VR
{
    public class SteamVR_RenderModel : MonoBehaviour
    {
            ...
            public CVRRenderModels instance
            {
                get
                {
                    if (_instance == null && !failedLoadInterface)
                    {
                        ...
                        if (_instance == null)
                        {
                            MelonLoader.MelonLogger.Error("[SteamVR] Failed to load IVRRenderModels interface version " + OpenVR.IVRRenderModels_Version);
        ...
        public void UpdateModel()
        {
            ...
            if (capacity <= 1)
            {
                MelonLoader.MelonLogger.Error("[SteamVR] Failed to get render model name for tracked object " + index);
                return;
            }
            ...
            using (RenderModelInterfaceHolder holder = new RenderModelInterfaceHolder())
            {
                ...
                while (true)
                {
                    ...
                    for (int renderModelNameIndex = 0; renderModelNameIndex < renderModelNames.Length; renderModelNameIndex++)
                    {
                        ...
                        //MelonLoader.MelonLogger.Msg("[SteamVR] renderModels.LoadRenderModel_Async(" + renderModelNames[renderModelNameIndex] + ": " + error.ToString());
        ...
        private bool SetModel(string renderModelName)
        {
            ...
            using (var holder = new RenderModelInterfaceHolder())
            {
                if (createComponents)
                {
                    ...
                    MelonLoader.MelonLogger.Msg("[SteamVR] [" + gameObject.name + "] Render model does not support components, falling back to single mesh.");
                }
                ...
                if (!string.IsNullOrEmpty(renderModelName))
                {
                    ...
                    if (model == null || model.mesh == null)
                    {
                        ...
                        if (verbose)
                            MelonLoader.MelonLogger.Msg("[SteamVR] Loading render model " + renderModelName);
        ...
        RenderModel LoadRenderModel(CVRRenderModels renderModels, string renderModelName, string baseName)
        {
            ...
            {
                MelonLoader.MelonLogger.Error(string.Format("[SteamVR] Failed to load render model {0} - {1}", renderModelName, error.ToString()));
                return null;
            }
            ...
            if (material == null || material.mainTexture == null)
            {
                ...
                {
                    MelonLoader.MelonLogger.Msg("[SteamVR] Failed to load render model texture for render model " + renderModelName + ". Error: " + error.ToString());
                }
            }
        ...
        private bool LoadComponents(RenderModelInterfaceHolder holder, string renderModelName)
        {
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                {
                    if (verbose)
                        MelonLoader.MelonLogger.Msg("[SteamVR] Loading render model " + componentRenderModelName);
        ...
        void OnEnable()
        {
            ...
            {
                MelonLoader.MelonLogger.Msg("[SteamVR] " + modelOverrideWarning);
                enabled = false;
                return;
            }
