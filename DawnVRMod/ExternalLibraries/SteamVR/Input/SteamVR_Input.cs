...
namespace Valve.VR
{
    public partial class SteamVR_Input
    {
        ...
        public static void Initialize(bool force = false)
        {
            ...
            //MelonLoader.MelonLogger.Msg("[SteamVR] Initializing SteamVR input...");
            initializing = true;
            ...
            if (SteamVR_Settings.instance.activateFirstActionSetOnStart)
            {
                ...
                {
                    MelonLoader.MelonLogger.Error("[SteamVR] No action sets to activate.");
                }
            }
            ...
            initialized = true;
            initializing = false;
            //MelonLoader.MelonLogger.Msg("[SteamVR] Input initialization complete.");
        }
        ...
        public static T[] GetActions<T>() where T : SteamVR_Action
        {
            ...
            {
                MelonLoader.MelonLogger.Msg("[SteamVR] Wrong type.");
            }
        ...
        public static void IdentifyActionsFile(bool showLogs = true)
        {
            ...
            if (File.Exists(fullPath))
            {
                ...
                {
                    MelonLoader.MelonLogger.Error("[SteamVR] Could not instantiate OpenVR Input interface.");
                    return;
                }

                EVRInputError err = OpenVR.Input.SetActionManifestPath(fullPath);
                if (err != EVRInputError.None)
                    MelonLoader.MelonLogger.Error("[SteamVR] Error loading action manifest into SteamVR: " + err.ToString());
                else
                {
                    int numActions = 0;
                    if (SteamVR_Input.actions != null)
                    {
                        ...
                        if (showLogs)
                            MelonLoader.MelonLogger.Msg(string.Format("[SteamVR] Successfully loaded {0} actions from action manifest into SteamVR ({1})", numActions, fullPath));
                    }
                    else
                    {
                        if (showLogs)
                            MelonLoader.MelonLogger.Warning("[SteamVR] No actions found, but the action manifest was loaded. This usually means you haven't generated actions. Window -> SteamVR Input -> Save and Generate.");
                    }
                }
            }
            else
            {
                if (showLogs)
                    MelonLoader.MelonLogger.Error("[SteamVR] Could not find actions file at: " + fullPath);
            }
        }
        ...
        public static bool CreateEmptyActionsFile(bool completelyEmpty = false)
        {
            ...
            {
                MelonLoader.MelonLogger.Error("[SteamVR] Actions file already exists in project root: " + actionsFilePath);
                return false;
            }
        ...
        public static bool InitializeFile(bool force = false, bool showErrors = true)
        {
            ...
            {
                if (showErrors)
                    MelonLoader.MelonLogger.Error("[SteamVR] Actions file does not exist in project root: " + actionsFilePath);
        ...
        private static void CheckSetup()
        {
            if (checkingSetup == false && openingSetup == false && (SteamVR_Input.actions == null || SteamVR_Input.actions.Length == 0))
            {
                ...
                {
                    MelonLoader.MelonLogger.Error("[SteamVR] This version of SteamVR will not work if you do not create and generate actions. Please open the SteamVR Input window or downgrade to version 1.2.3 (on github)");
                }
                checkingSetup = false;
            }
        }
