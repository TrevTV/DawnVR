        ...
        public string[] GetFilesToCopy(bool throwErrors = false)
        {
            ...
            foreach (var binding in default_bindings)
            {
                ...
                else
                {
                    if (throwErrors)
                    {
                        MelonLoader.MelonLogger.Error("[SteamVR] Could not bind binding file specified by the actions.json manifest: " + bindingPath);
                    }
                }
            }
            ...
            foreach (string file in files)
            {
                ...
                if (exists)
                {
                    if (overwrite)
                    {
                        ...
                        MelonLoader.MelonLogger.Msg("[SteamVR] Copied (overwrote) SteamVR Input file at path: " + newFilePath);
                    }
                    else
                    {
                        MelonLoader.MelonLogger.Msg("[SteamVR] Skipped writing existing file at path: " + newFilePath);
                    }
                }
                else
                {
                    ...
                    MelonLoader.MelonLogger.Msg("[SteamVR] Copied SteamVR Input file to folder: " + newFilePath);
                }
    ...
    public class SteamVR_Input_ActionFile_LocalizationItem
    {
        ...
        public SteamVR_Input_ActionFile_LocalizationItem(Dictionary<string, string> dictionary)
        {
            ...
            if (dictionary.ContainsKey(languageTagKeyName))
                ...
                MelonLoader.MelonLogger.Msg("[SteamVR] Input: Error in actions file, no language_tag in localization array item.");
