...
namespace Valve.VR
{
    public class SteamVR : System.IDisposable
    {
        ...
        private static SteamVR CreateInstance()
        {
            ...
            {
                MelonLoader.MelonLogger.Error("[SteamVR] " + e);
                SteamVR_Events.Initialized.Send(false);
                return null;
            }
        ...
        static void ReportError(EVRInitError error)
        {
            switch (error)
            {
                ...
                case EVRInitError.VendorSpecific_UnableToConnectToOculusRuntime:
                    MelonLoader.MelonLogger.Warning("[SteamVR] Initialization Failed!  Make sure device is on, Oculus runtime is installed, and OVRService_*.exe is running.");
                    break;
                case EVRInitError.Init_VRClientDLLNotFound:
                    MelonLoader.MelonLogger.Warning("[SteamVR] Drivers not found!  They can be installed via Steam under Library > Tools.  Visit http://steampowered.com to install Steam.");
                    break;
                case EVRInitError.Driver_RuntimeOutOfDate:
                    MelonLoader.MelonLogger.Warning("[SteamVR] Initialization Failed!  Make sure device's runtime is up to date.");
                    break;
                default:
                    MelonLoader.MelonLogger.Warning("[SteamVR] " + OpenVR.GetStringForHmdError(error));
                    break;
            }
        }
        ...
        public static bool InitializeTemporarySession(bool initInput = false)
        {
            if (Application.isEditor)
            {
                ...
                if (needsInit)
                {
                    ...
                    if (error != EVRInitError.None)
                    {
                        MelonLoader.MelonLogger.Error("[SteamVR] Error during OpenVR Init: " + error.ToString());
                        return false;
                    }
        ...
        public static void ShowBindingsForEditor()
        {
            ...
            if (bindingFlagError != Valve.VR.EVRSettingsError.None)
                MelonLoader.MelonLogger.Error("[SteamVR] Error turning on the debug input binding flag in steamvr: " + bindingFlagError.ToString());
        ...
        private static string GetManifestFile()
        {
            ...
            if (File.Exists(fullPath))
            {
                ...
                {
                    MelonLoader.MelonLogger.Msg("[SteamVR] Deleting existing VRManifest because it has a different app key.");
                ...
                {
                    MelonLoader.MelonLogger.Msg("[SteamVR] Deleting existing VRManifest because it has a different action manifest path:" +
            ...
            if (File.Exists(fullPath) == false)
            {
                ...
                if (SteamVR_Input.actionFile != null)
                {
                    ...
                    {
                        MelonLoader.MelonLogger.Error("[SteamVR] Mismatch in available binding files.");
                    }
                }
                else
                {
                    MelonLoader.MelonLogger.Error("[SteamVR] Could not load actions file.");
                }
                */
        ...
        private static void IdentifyEditorApplication(bool showLogs = true)
        {
            ...
            {
                MelonLoader.MelonLogger.Error("[SteamVR] Critical Error identifying application. EditorAppKey is null or empty. Input may not work.");
                return;
            }
            ...
            if (addManifestErr != EVRApplicationError.None)
                MelonLoader.MelonLogger.Error("[SteamVR] Error adding vr manifest file: " + addManifestErr.ToString());
            else
            {
                if (showLogs)
                    MelonLoader.MelonLogger.Msg("[SteamVR] Successfully added VR manifest to SteamVR");
            }
            ...
            if (applicationIdentifyErr != EVRApplicationError.None)
                MelonLoader.MelonLogger.Error("[SteamVR] Error identifying application: " + applicationIdentifyErr.ToString());
            else
            {
                if (showLogs)
                    MelonLoader.MelonLogger.Msg(string.Format("[SteamVR] Successfully identified process as editor project to SteamVR ({0})", SteamVR_Settings.instance.editorAppKey));
            }
        }
        ...
        {
            hmd = OpenVR.System;
            MelonLoader.MelonLogger.Msg("[SteamVR] Initialized. Connected to " + hmd_TrackingSystemName + ":" + hmd_SerialNumber);
